...
def run_tests(testsuite):
    ...
    def run_group(group):
        ...
        for test in group["tests"]:
            process_result = \
                subprocess.run([executable, "-base", "linear", test["filename"]])
                               #stdout=subprocess.DEVNULL, \
                               #stderr=subprocess.DEVNULL)
            result = True
            print("EXIT CODE:" + str(process_result.returncode))
...
def main():
    # Check current directory - support for either project root or test root
